...
from settings import TILE_SIZE, PLAYER_WIDTH_1, PLAYER_HEIGHT_1
...
class Player:
    def __init__(self, x, y):
        ...
        self.rect = pygame.Rect(x, y, PLAYER_WIDTH_1, PLAYER_HEIGHT_1)

        # 画像は元サイズのまま
        self.image = pygame.transform.scale(PLAYER_IMAGE, (PLAYER_WIDTH_1, PLAYER_HEIGHT_1))  # 画像をリサイズ

        self.velocity = pygame.Vector2(0, 0)
        self.holding = None  # 持っているアイテム
        self.map = Map()  # マップ情報を保持
        self.tile_size = TILE_SIZE
        self.player_size = PLAYER_WIDTH_1

        self.butttun_push_cnt = 0
    ...
    def handle_input(self, keys):
        ...
        '''
        if keys[pygame.K_a]:
            self.velocity.x = -PLAYER_SPEED#*20
        if keys[pygame.K_d]:
            self.velocity.x = PLAYER_SPEED#*20

    def push_cnt(self):
        return self.butttun_push_cnt
